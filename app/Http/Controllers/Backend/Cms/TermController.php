<?php

namespace App\Http\Controllers\Backend\Cms;
use App\Models\Term;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\DataTables\Cms\TermDataTable;
use Illuminate\Support\Facades\{DB, Session};
class TermController extends Controller {
    public function index(TermDataTable $about) {
        return $about->render('admin.cms.term.index', ['title' => 'term']);
    }

    public function store(Request $request) {
        try {
            $term = Term::create($request->except('_token'));
            Session::flash('message', 'data create success');
            Session::flash('alert-class', 'alert-success');
            return redirect()->back();
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Try again later Something went wrong');
        }
    }

    public function update(Request $request, $id) {
        try {
            $term = Term::findorfail($id);
            $term->update($request->except('_token'));
            Session::flash('message', 'data updated success');
            Session::flash('alert-class', 'alert-success');
            return redirect()->back();
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Try again later Something went wrong');
        }
    }

    public function destroy(string $id) {
        try {
            DB::beginTransaction();
            $term = Term::findOrFail($id);
            $term->delete();
            DB::commit();

        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Try again later Something went wrong');
        }
        Session::flash('message', 'data Deleted success');
        Session::flash('alert-class', 'alert-danger');
        return redirect()->back()->with('success', 'Delete Successfully term');
    }
}