                ...
                <form action="{{ route('button.update', $button->id) }}" method="POST">
                    ...
                    <div class="p-1 form-group" id="typePaymentsGroup">
                        ...
                        <select name="typePaymernts" id="typePayments" class="form-control">
                            ...
                            <option value="paypal" {{ old('typePaymernts', $button->typePaymernts) == 'paypal' ?
                                'selected' : '' }}>PayPal
                            </option>
                            <option value="checkout" {{ old('typePaymernts', $button->typePaymernts) == 'checkout' ?
                                'selected' : ''
                                }}>2Checkout</option>
                        </select>
                        @error('typePaymernts')
                    ...
                    <div class="p-1 form-group">
                        ...
                        <select name="status" class="form-control">
                            ...
                            <option value="1" {{ old('status', $button->status)=='1' ? 'selected' : '' }}>Active
                            </option>
                            <option value="0" {{ old('status', $button->status)=='0' ? 'selected' : '' }}>Not Active
                            </option>
                        </select>
                        @error('status')
                        <div class="alert alert-danger">{{ $message }}</div>
